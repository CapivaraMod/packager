...
<GuideModal bind:visible={guideVisible} />

<button id="guia-btn" type="button" on:click={() => guideVisible = true} aria-haspopup="dialog" aria-expanded={guideVisible}>
  {$_("p4.guia")}
</button>
<main aria-hidden={modalVisible || guideVisible} class:is-not-safari={!isSafari}>
...
<style>
  ...
  :global(html) {
    scrollbar-color: #3DA00B #3c3c3c30;
  }
  main {
    padding-bottom: 10px;
  }
